<template>
  <div class="page flex-col">
    <div class="box_1 flex-col">
      <div class="box_2 flex-row">
        <span class="text_1">12:46</span>
        <img
          class="label_1"
          referrerpolicy="no-referrer"
          src="./assets/img/FigmaDDSSlicePNG94cb71e30d2d996f553895c465a05218.png"
        />
        <img
          class="label_2"
          referrerpolicy="no-referrer"
          src="./assets/img/FigmaDDSSlicePNGbc8b2068f088ec7c32164fd3bab7d09a.png"
        />
        <img
          class="label_3"
          referrerpolicy="no-referrer"
          src="./assets/img/FigmaDDSSlicePNG9d5b301c9341e350bc3d1e6f878ed4b8.png"
        />
      </div>
      <div class="text-wrapper_1 flex-col"><span class="text_2">http://www.haiyangcaijing.com</span></div>
    </div>
    <div class="box_3 flex-col">
      <div class="group_1 flex-row">
        <div class="image-wrapper_1 flex-col justify-between">
          <img
            class="image_1"
            referrerpolicy="no-referrer"
            src="./assets/img/FigmaDDSSlicePNGc7c8243b244fed5dd5ea65509a4bf192.png"
          />
          <img
            class="image_2"
            referrerpolicy="no-referrer"
            src="./assets/img/FigmaDDSSlicePNGa248b93bac5dd25c5ba4e2eff60f7753.png"
          />
        </div>
        <div class="group_2 flex-col">
          <div class="image-wrapper_2 flex-col">
            <img
              class="label_4"
              referrerpolicy="no-referrer"
              src="./assets/img/FigmaDDSSlicePNGc91b60cc0d136034d7e1cde253d50a91.png"
            />
          </div>
        </div>
        <img
          class="label_5"
          referrerpolicy="no-referrer"
          src="./assets/img/FigmaDDSSlicePNG85213edb0f33d45b7ecd052df3ec843a.png"
        />
      </div>
      <div class="group_3 flex-row">
        <span class="text_3">海洋财经</span>
        <img
          class="thumbnail_1"
          referrerpolicy="no-referrer"
          src="./assets/img/FigmaDDSSlicePNG378c59d9ff2c6ab7928bcb7dacf26fb5.png"
        />
        <div class="image-text_1 flex-row justify-between">
          <span class="text-group_1">期货一对一服务</span>
          <img
            class="thumbnail_2"
            referrerpolicy="no-referrer"
            src="./assets/img/FigmaDDSSlicePNG378c59d9ff2c6ab7928bcb7dacf26fb5.png"
          />
        </div>
      </div>
      <div class="group_4 flex-col"></div>
    </div>
    <div class="box_4 flex-col">
      <img
        class="image_3"
        referrerpolicy="no-referrer"
        src="./assets/img/FigmaDDSSlicePNGaed6b8de8e053613b1875e49619a90ee.png"
      />
      <div class="box_5 flex-row">
        <div class="image-text_2 flex-row justify-between">
          <div class="section_1 flex-col">
            <div class="text-wrapper_2 flex-col"><span class="text_4">股票顾问</span></div>
          </div>
          <div class="section_2 flex-col justify-between">
            <div class="text-group_2 flex-col">
              <span class="text_5">资深顾问张三...</span>
              <span class="text_6">已帮助</span>
              <span class="text_7">2.7万+人</span>
            </div>
            <div class="box_6 flex-row">
              <div class="image-text_3 flex-row justify-between">
                <img
                  class="label_6"
                  referrerpolicy="no-referrer"
                  src="./assets/img/7b2959f48b0e42d0ba5cfb8f44c62c48_mergeImage.png"
                />
                <span class="text-group_3">咨询</span>
              </div>
              <div class="image-text_4 flex-row justify-between">
                <img
                  class="label_6"
                  referrerpolicy="no-referrer"
                  src="./assets/img/7b2959f48b0e42d0ba5cfb8f44c62c48_mergeImage.png"
                />
                <span class="text-group_3">咨询</span>
              </div>
            </div>
          </div>
        </div>
      </div>
      <div class="box_7 flex-row justify-between">
        <div class="group_5 flex-row">
          <div class="image-text_5 flex-row justify-between">
            <div class="block_1 flex-col">
              <div class="text-wrapper_3 flex-col"><span class="text_8">股票顾问</span></div>
            </div>
            <div class="block_2 flex-col justify-between">
              <div class="text-group_4 flex-col">
                <span class="text_9">资深顾问张三...</span>
                <span class="text_10">已帮助</span>
                <span class="text_11">2.7万+人</span>
              </div>
              <div class="block_3 flex-row">
                <div class="image-text_6 flex-row justify-between">
                  <img
                    class="label_7"
                    referrerpolicy="no-referrer"
                    src="./assets/img/152698e1a95c49d89c9b93819104b31a_mergeImage.png"
                  />
                  <span class="text-group_5">咨询</span>
                </div>
                <div class="image-text_7 flex-row justify-between">
                  <img
                    class="label_7"
                    referrerpolicy="no-referrer"
                    src="./assets/img/152698e1a95c49d89c9b93819104b31a_mergeImage.png"
                  />
                  <span class="text-group_5">咨询</span>
                </div>
              </div>
            </div>
          </div>
        </div>
        <div class="group_6 flex-row">
          <div class="image-text_8 flex-row justify-between">
            <div class="section_3 flex-col">
              <div class="text-wrapper_4 flex-col"><span class="text_12">股票顾问</span></div>
            </div>
            <div class="section_4 flex-col justify-between">
              <div class="text-group_6 flex-col">
                <span class="text_13">资深顾问张三...</span>
                <span class="text_14">已帮助</span>
                <span class="text_15">2.7万+人</span>
              </div>
              <div class="box_8 flex-row">
                <div class="image-text_9 flex-row justify-between">
                  <img
                    class="label_8"
                    referrerpolicy="no-referrer"
                    src="./assets/img/5acf5c5d20404a4595fcd782005700e4_mergeImage.png"
                  />
                  <span class="text-group_7">咨询</span>
                </div>
                <div class="image-text_10 flex-row justify-between">
                  <img
                    class="label_8"
                    referrerpolicy="no-referrer"
                    src="./assets/img/5acf5c5d20404a4595fcd782005700e4_mergeImage.png"
                  />
                  <span class="text-group_7">咨询</span>
                </div>
              </div>
            </div>
          </div>
        </div>
      </div>
      <div class="box_9 flex-row">
        <div class="image-text_11 flex-row justify-between">
          <img
            class="thumbnail_3"
            referrerpolicy="no-referrer"
            src="./assets/img/FigmaDDSSlicePNGc03324d3b69e5a98ffe9ebfde912cc6d.png"
          />
          <span class="text-group_8">顾问动态：</span>
        </div>
        <span class="text_16">郭瑾顾问刚刚回答了一条问题：《有冠心病还可以买哪...</span>
      </div>
      <span class="text_17">合肥顾问</span>
      <div class="box_10 flex-col">
        <div class="section_5 flex-row">
          <div class="text-group_9 flex-col justify-between">
            <span class="text_18">优质顾问</span>
            <span class="text_19">最具影响力的理财顾问</span>
          </div>
        </div>
        <div class="section_6 flex-row">
          <div class="image-text_12 flex-row justify-between">
            <img
              class="image_4"
              referrerpolicy="no-referrer"
              src="./assets/img/FigmaDDSSlicePNG8e6104d3ccde290b76a1941918f9eb50.png"
            />
            <div class="block_4 flex-col justify-between">
              <span class="text-group_10">资深顾问祝</span>
              <div class="text-wrapper_5 flex-col"><span class="text_20">股票顾问</span></div>
            </div>
          </div>
          <div class="text-wrapper_6 flex-col"><span class="text_21">帮助2.7万</span></div>
          <div class="image-wrapper_3 flex-col">
            <img
              class="label_9"
              referrerpolicy="no-referrer"
              src="./assets/img/FigmaDDSSlicePNG854cf5844a8ddcd486c61b1cf69fb52c.png"
            />
          </div>
          <div class="image-wrapper_4 flex-col">
            <img
              class="label_10"
              referrerpolicy="no-referrer"
              src="./assets/img/FigmaDDSSlicePNG7235843c577a660f1e6f88cc5aa9a3f6.png"
            />
          </div>
        </div>
        <div class="section_7 flex-row justify-between">
          <div class="image-text_13 flex-row justify-between">
            <img
              class="thumbnail_4"
              referrerpolicy="no-referrer"
              src="./assets/img/FigmaDDSSlicePNGc0aade8c1037e19137e79105b3c92f59.png"
            />
            <span class="text-group_11">好评3849</span>
          </div>
          <div class="image-text_14 flex-row justify-between">
            <img
              class="thumbnail_5"
              referrerpolicy="no-referrer"
              src="./assets/img/FigmaDDSSlicePNGb42c80e173419edeb30ef9937b8e2ed3.png"
            />
            <span class="text-group_12">浏览量48万+</span>
          </div>
        </div>
        <div class="section_8 flex-row">
          <div class="image-text_15 flex-row justify-between">
            <img
              class="image_5"
              referrerpolicy="no-referrer"
              src="./assets/img/FigmaDDSSlicePNG4983fb78496a9eea89b68efef8fcddff.png"
            />
            <div class="box_11 flex-col justify-between">
              <span class="text-group_13">资深顾问祝</span>
              <div class="text-wrapper_7 flex-col"><span class="text_22">股票顾问</span></div>
            </div>
          </div>
          <div class="text-wrapper_8 flex-col"><span class="text_23">帮助2.7万</span></div>
          <div class="image-wrapper_5 flex-col">
            <img
              class="label_11"
              referrerpolicy="no-referrer"
              src="./assets/img/FigmaDDSSlicePNG854cf5844a8ddcd486c61b1cf69fb52c.png"
            />
          </div>
          <div class="image-wrapper_6 flex-col">
            <img
              class="label_12"
              referrerpolicy="no-referrer"
              src="./assets/img/FigmaDDSSlicePNG7235843c577a660f1e6f88cc5aa9a3f6.png"
            />
          </div>
        </div>
        <div class="section_9 flex-row justify-between">
          <div class="image-text_16 flex-row justify-between">
            <img
              class="thumbnail_6"
              referrerpolicy="no-referrer"
              src="./assets/img/FigmaDDSSlicePNGc0aade8c1037e19137e79105b3c92f59.png"
            />
            <span class="text-group_14">好评3849</span>
          </div>
          <div class="image-text_17 flex-row justify-between">
            <img
              class="thumbnail_7"
              referrerpolicy="no-referrer"
              src="./assets/img/FigmaDDSSlicePNGb42c80e173419edeb30ef9937b8e2ed3.png"
            />
            <span class="text-group_15">浏览量48万+</span>
          </div>
        </div>
        <div class="section_10 flex-row">
          <div class="image-text_18 flex-row justify-between">
            <img
              class="image_6"
              referrerpolicy="no-referrer"
              src="./assets/img/FigmaDDSSlicePNG40bbc3324a388f3fa6c9d02cc61846c8.png"
            />
            <div class="box_12 flex-col justify-between">
              <span class="text-group_16">资深顾问祝</span>
              <div class="text-wrapper_9 flex-col"><span class="text_24">股票顾问</span></div>
            </div>
          </div>
          <div class="text-wrapper_10 flex-col"><span class="text_25">帮助2.7万</span></div>
          <div class="image-wrapper_7 flex-col">
            <img
              class="label_13"
              referrerpolicy="no-referrer"
              src="./assets/img/FigmaDDSSlicePNG854cf5844a8ddcd486c61b1cf69fb52c.png"
            />
          </div>
          <div class="image-wrapper_8 flex-col">
            <img
              class="label_14"
              referrerpolicy="no-referrer"
              src="./assets/img/FigmaDDSSlicePNG7235843c577a660f1e6f88cc5aa9a3f6.png"
            />
          </div>
        </div>
        <div class="section_11 flex-row justify-between">
          <div class="image-text_19 flex-row justify-between">
            <img
              class="thumbnail_8"
              referrerpolicy="no-referrer"
              src="./assets/img/FigmaDDSSlicePNGc0aade8c1037e19137e79105b3c92f59.png"
            />
            <span class="text-group_17">好评3849</span>
          </div>
          <div class="image-text_20 flex-row justify-between">
            <img
              class="thumbnail_9"
              referrerpolicy="no-referrer"
              src="./assets/img/FigmaDDSSlicePNGb42c80e173419edeb30ef9937b8e2ed3.png"
            />
            <span class="text-group_18">浏览量48万+</span>
          </div>
        </div>
        <div class="section_12 flex-row">
          <div class="image-text_21 flex-row justify-between">
            <img
              class="image_7"
              referrerpolicy="no-referrer"
              src="./assets/img/FigmaDDSSlicePNG78c5c28eece856da63fa62bce0e85200.png"
            />
            <div class="block_5 flex-col justify-between">
              <span class="text-group_19">资深顾问祝</span>
              <div class="text-wrapper_11 flex-col"><span class="text_26">股票顾问</span></div>
            </div>
          </div>
          <div class="text-wrapper_12 flex-col"><span class="text_27">帮助2.7万</span></div>
          <div class="image-wrapper_9 flex-col">
            <img
              class="label_15"
              referrerpolicy="no-referrer"
              src="./assets/img/FigmaDDSSlicePNG854cf5844a8ddcd486c61b1cf69fb52c.png"
            />
          </div>
          <div class="image-wrapper_10 flex-col">
            <img
              class="label_16"
              referrerpolicy="no-referrer"
              src="./assets/img/FigmaDDSSlicePNG7235843c577a660f1e6f88cc5aa9a3f6.png"
            />
          </div>
        </div>
        <div class="section_13 flex-row justify-between">
          <div class="image-text_22 flex-row justify-between">
            <img
              class="thumbnail_10"
              referrerpolicy="no-referrer"
              src="./assets/img/FigmaDDSSlicePNGc0aade8c1037e19137e79105b3c92f59.png"
            />
            <span class="text-group_20">好评3849</span>
          </div>
          <div class="image-text_23 flex-row justify-between">
            <img
              class="thumbnail_11"
              referrerpolicy="no-referrer"
              src="./assets/img/FigmaDDSSlicePNGb42c80e173419edeb30ef9937b8e2ed3.png"
            />
            <span class="text-group_21">浏览量48万+</span>
          </div>
        </div>
        <div class="section_14 flex-row">
          <div class="image-text_24 flex-row justify-between">
            <img
              class="image_8"
              referrerpolicy="no-referrer"
              src="./assets/img/FigmaDDSSlicePNGe48407779c25c0d07ff0bf7fc97ff32f.png"
            />
            <div class="group_7 flex-col justify-between">
              <span class="text-group_22">资深顾问祝</span>
              <div class="text-wrapper_13 flex-col"><span class="text_28">股票顾问</span></div>
            </div>
          </div>
          <div class="text-wrapper_14 flex-col"><span class="text_29">帮助2.7万</span></div>
          <div class="image-wrapper_11 flex-col">
            <img
              class="label_17"
              referrerpolicy="no-referrer"
              src="./assets/img/FigmaDDSSlicePNG854cf5844a8ddcd486c61b1cf69fb52c.png"
            />
          </div>
          <div class="image-wrapper_12 flex-col">
            <img
              class="label_18"
              referrerpolicy="no-referrer"
              src="./assets/img/FigmaDDSSlicePNG7235843c577a660f1e6f88cc5aa9a3f6.png"
            />
          </div>
        </div>
        <div class="section_15 flex-row justify-between">
          <div class="image-text_25 flex-row justify-between">
            <img
              class="thumbnail_12"
              referrerpolicy="no-referrer"
              src="./assets/img/FigmaDDSSlicePNGc0aade8c1037e19137e79105b3c92f59.png"
            />
            <span class="text-group_23">好评3849</span>
          </div>
          <div class="image-text_26 flex-row justify-between">
            <img
              class="thumbnail_13"
              referrerpolicy="no-referrer"
              src="./assets/img/FigmaDDSSlicePNGb42c80e173419edeb30ef9937b8e2ed3.png"
            />
            <span class="text-group_24">浏览量48万+</span>
          </div>
        </div>
        <div class="section_16 flex-row">
          <div class="image-text_27 flex-row justify-between">
            <img
              class="image_9"
              referrerpolicy="no-referrer"
              src="./assets/img/FigmaDDSSlicePNGc33e82e08b71a16193082948414e56d5.png"
            />
            <div class="box_13 flex-col justify-between">
              <span class="text-group_25">资深顾问祝</span>
              <div class="text-wrapper_15 flex-col"><span class="text_30">股票顾问</span></div>
            </div>
          </div>
          <div class="text-wrapper_16 flex-col"><span class="text_31">帮助2.7万</span></div>
          <div class="image-wrapper_13 flex-col">
            <img
              class="label_19"
              referrerpolicy="no-referrer"
              src="./assets/img/FigmaDDSSlicePNG854cf5844a8ddcd486c61b1cf69fb52c.png"
            />
          </div>
          <div class="image-wrapper_14 flex-col">
            <img
              class="label_20"
              referrerpolicy="no-referrer"
              src="./assets/img/FigmaDDSSlicePNG7235843c577a660f1e6f88cc5aa9a3f6.png"
            />
          </div>
        </div>
        <div class="section_17 flex-row justify-between">
          <div class="image-text_28 flex-row justify-between">
            <img
              class="thumbnail_14"
              referrerpolicy="no-referrer"
              src="./assets/img/FigmaDDSSlicePNGc0aade8c1037e19137e79105b3c92f59.png"
            />
            <span class="text-group_26">好评3849</span>
          </div>
          <div class="image-text_29 flex-row justify-between">
            <img
              class="thumbnail_15"
              referrerpolicy="no-referrer"
              src="./assets/img/FigmaDDSSlicePNGb42c80e173419edeb30ef9937b8e2ed3.png"
            />
            <span class="text-group_27">浏览量48万+</span>
          </div>
        </div>
      </div>
    </div>
    <div class="box_14 flex-col">
      <div class="group_8 flex-row justify-between">
        <span class="text_32">最新问答</span>
        <div class="image-text_30 flex-row">
          <span class="text-group_28">更多</span>
          <img
            class="thumbnail_16"
            referrerpolicy="no-referrer"
            src="./assets/img/FigmaDDSSlicePNG378b835325af5a684c617c3fec63849a.png"
          />
        </div>
      </div>
      <div class="group_9 flex-col"><div class="section_18 flex-col"></div></div>
      <div class="group_10 flex-row justify-between">
        <div class="block_6 flex-col"></div>
        <div class="block_7 flex-col justify-between">
          <div class="box_15 flex-col">
            <div class="block_8 flex-row justify-between">
              <div class="image-text_31 flex-row justify-between">
                <span class="text-group_29">资深吴经理</span>
                <div class="block_9 flex-col"></div>
              </div>
              <span class="text_33">2024-03-07</span>
            </div>
            <div class="block_10 flex-col">
              <div class="box_16 flex-row justify-between">
                <div class="text-wrapper_17 flex-col"><span class="text_34">问</span></div>
                <span class="text_35">哪位老师能讲讲恒力期货营业部在哪里?谢谢</span>
              </div>
              <div class="box_17 flex-row justify-between">
                <div class="text-wrapper_18 flex-col"><span class="text_36">答</span></div>
                <span class="text_37">
                  您好!恒力期货营业部在全国有14个分支机构，投资者可在官网查询选择所在城市营业部查看具体地址。恒力期货...
                </span>
              </div>
            </div>
          </div>
          <div class="box_18 flex-col">
            <div class="section_19 flex-row justify-between">
              <div class="image-text_32 flex-row justify-between">
                <span class="text-group_30">资深吴经理</span>
                <div class="box_19 flex-col"></div>
              </div>
              <span class="text_38">2024-03-07</span>
            </div>
            <div class="section_20 flex-col">
              <div class="group_11 flex-row justify-between">
                <div class="text-wrapper_19 flex-col"><span class="text_39">问</span></div>
                <span class="text_40">哪位老师能讲讲恒力期货营业部在哪里?谢谢</span>
              </div>
              <div class="group_12 flex-row justify-between">
                <div class="text-wrapper_20 flex-col"><span class="text_41">答</span></div>
                <span class="text_42">
                  您好!恒力期货营业部在全国有14个分支机构，投资者可在官网查询选择所在城市营业部查看具体地址。恒力期货...
                </span>
              </div>
            </div>
          </div>
          <div class="box_20 flex-col">
            <div class="block_11 flex-row justify-between">
              <div class="image-text_33 flex-row justify-between">
                <span class="text-group_31">资深吴经理</span>
                <div class="group_13 flex-col"></div>
              </div>
              <span class="text_43">2024-03-07</span>
            </div>
            <div class="block_12 flex-col">
              <div class="block_13 flex-row justify-between">
                <div class="text-wrapper_21 flex-col"><span class="text_44">问</span></div>
                <span class="text_45">哪位老师能讲讲恒力期货营业部在哪里?谢谢</span>
              </div>
              <div class="block_14 flex-row justify-between">
                <div class="text-wrapper_22 flex-col"><span class="text_46">答</span></div>
                <span class="text_47">
                  您好!恒力期货营业部在全国有14个分支机构，投资者可在官网查询选择所在城市营业部查看具体地址。恒力期货...
                </span>
              </div>
            </div>
          </div>
        </div>
      </div>
      <img
        class="image_10"
        referrerpolicy="no-referrer"
        src="./assets/img/FigmaDDSSlicePNG72f411431b2b6194adf3ddc359118984.png"
      />
      <img
        class="image_11"
        referrerpolicy="no-referrer"
        src="./assets/img/FigmaDDSSlicePNGe5965ce6dc5972ff9e3d341c028979e8.png"
      />
      <img
        class="image_12"
        referrerpolicy="no-referrer"
        src="./assets/img/FigmaDDSSlicePNGd7fb71ee5f1f7d7cbb18ca7681bc8b62.png"
      />
    </div>
    <div class="box_21 flex-col">
      <div class="box_22 flex-row justify-between">
        <span class="text_48">最新问答</span>
        <div class="text-wrapper_23 flex-col"><span class="text_49">换一批</span></div>
      </div>
      <div class="box_23 flex-col"><div class="block_15 flex-col"></div></div>
      <div class="box_24 flex-row justify-between">
        <div class="group_14 flex-col">
          <div class="group_15 flex-col"></div>
          <div class="group_16 flex-col">
            <div class="box_25 flex-col"></div>
            <div class="box_26 flex-col"></div>
            <div class="box_27 flex-col"></div>
            <div class="box_28 flex-col"></div>
          </div>
          <div class="group_17 flex-col"></div>
        </div>
        <div class="text-wrapper_24 flex-col">
          <span class="text_50">哪位老师能讲讲恒力期货营业部在哪里?谢谢</span>
          <span class="text_51">
            您好!恒力期货营业部在全国有14个分支机构，投资者可在官网查询选择所在城市营业部查看具体地址。恒力期货...
          </span>
          <span class="text_52">哪位老师能讲讲恒力期货营业部在哪里?谢谢</span>
          <span class="text_53">
            您好!恒力期货营业部在全国有14个分支机构，投资者可在官网查询选择所在城市营业部查看具体地址。恒力期货...
          </span>
          <span class="text_54">哪位老师能讲讲恒力期货营业部在哪里?谢谢</span>
          <span class="text_55">
            您好!恒力期货营业部在全国有14个分支机构，投资者可在官网查询选择所在城市营业部查看具体地址。恒力期货...
          </span>
          <span class="text_56">哪位老师能讲讲恒力期货营业部在哪里?谢谢</span>
          <span class="text_57">
            您好!恒力期货营业部在全国有14个分支机构，投资者可在官网查询选择所在城市营业部查看具体地址。恒力期货...
          </span>
          <span class="text_58">哪位老师能讲讲恒力期货营业部在哪里?谢谢</span>
          <span class="text_59">
            您好!恒力期货营业部在全国有14个分支机构，投资者可在官网查询选择所在城市营业部查看具体地址。恒力期货...
          </span>
          <span class="text_60">哪位老师能讲讲恒力期货营业部在哪里?谢谢</span>
        </div>
      </div>
      <span class="text_61">
        您好!恒力期货营业部在全国有14个分支机构，投资者可在官网查询选择所在城市营业部查看具体地址。恒力期货...
      </span>
    </div>
    <div class="box_29 flex-col">
      <div class="list_1 flex-row">
        <div class="image-text_34 flex-col justify-between" v-for="(item, index) in loopData0" :key="index">
          <img class="image_13" referrerpolicy="no-referrer" :src="item.lanhuimage0" />
          <span class="text-group_32" v-html="item.lanhutext0"></span>
        </div>
      </div>
      <div class="group_18 flex-col"></div>
      <span class="paragraph_1">
        京ICP备19048506号-2&nbsp;Copyright
        <br />
        &#64;&nbsp;2024&nbsp;中国海洋财经有限公司&nbsp;All&nbsp;Rights&nbsp;Reserved.
      </span>
    </div>
  </div>
</template>
<script>
export default {
  data() {
    return {
      loopData0: [
        {
          lanhuimage0: 'https://lanhu.oss-cn-beijing.aliyuncs.com/FigmaDDSSlicePNGfcd97fd90f0f75097d6661238ccc7d77.png',
          lanhutext0: '咨询'
        },
        {
          lanhuimage0: 'https://lanhu.oss-cn-beijing.aliyuncs.com/FigmaDDSSlicePNGfcd97fd90f0f75097d6661238ccc7d77.png',
          lanhutext0: '关注'
        }
      ],
      constants: {}
    };
  },
  methods: {}
};
</script>
<style scoped lang="css" src="./assets/index.rem.css" />