<template>
  <div class="page flex-col">
    <div class="group_1 flex-col">
      <div class="group_2 flex-row">
        <span class="text_1">12:46</span>
        <img
          class="label_1"
          referrerpolicy="no-referrer"
          src="./assets/img/FigmaDDSSlicePNG94cb71e30d2d996f553895c465a05218.png"
        />
        <img
          class="label_2"
          referrerpolicy="no-referrer"
          src="./assets/img/FigmaDDSSlicePNGbc8b2068f088ec7c32164fd3bab7d09a.png"
        />
        <img
          class="label_3"
          referrerpolicy="no-referrer"
          src="./assets/img/FigmaDDSSlicePNG9d5b301c9341e350bc3d1e6f878ed4b8.png"
        />
      </div>
      <div class="text-wrapper_1 flex-col"><span class="text_2">http://www.haiyangcaijing.com</span></div>
    </div>
    <div class="group_3 flex-col">
      <div class="section_1 flex-row justify-between">
        <div class="box_1 flex-col">
          <div class="image-wrapper_1 flex-col">
            <img
              class="label_4"
              referrerpolicy="no-referrer"
              src="./assets/img/FigmaDDSSlicePNGc91b60cc0d136034d7e1cde253d50a91.png"
            />
          </div>
        </div>
        <img
          class="label_5"
          referrerpolicy="no-referrer"
          src="./assets/img/FigmaDDSSlicePNG85213edb0f33d45b7ecd052df3ec843a.png"
        />
      </div>
      <div class="image-wrapper_2 flex-row">
        <img
          class="image_1"
          referrerpolicy="no-referrer"
          src="./assets/img/FigmaDDSSlicePNGae4bb62ad38ba4fb96fdf69012235348.png"
        />
      </div>
      <div class="image-wrapper_3 flex-row">
        <img
          class="image_2"
          referrerpolicy="no-referrer"
          src="./assets/img/FigmaDDSSlicePNG278475bcc372de80c93c57da41666b8f.png"
        />
      </div>
      <div class="image-wrapper_4 flex-row">
        <img
          class="image_3"
          referrerpolicy="no-referrer"
          src="./assets/img/FigmaDDSSlicePNGd41e257ff606eef7798cf3a7947c3580.png"
        />
      </div>
      <div class="section_2 flex-row">
        <div class="block_1 flex-row">
          <span class="text_3">寻宝</span>
          <div class="box_2 flex-col"></div>
          <span class="text_4">请输入关键词</span>
          <div class="text-wrapper_2 flex-col"><span class="text_5">问一问</span></div>
        </div>
      </div>
    </div>
    <div class="group_4 flex-col">
      <div class="text-wrapper_3 flex-row justify-between">
        <span class="text_6">优选文章</span>
        <span class="text_7">Preferred&nbsp;article</span>
      </div>
      <div class="group_5 flex-col"><div class="group_6 flex-col"></div></div>
      <span class="text_8">APP广告变现痛点，开发者如何解决APP广告变现痛点，开发者如何解决？</span>
      <span class="text_9">
        如何在技术上实现广告植入的正常姿势，从而达成用户、流量、收益三者之间的动态平衡？如何在技术上实现广告植入的正常姿势，从而达成用户、流量、收益三者？
      </span>
      <div class="group_7 flex-row justify-between">
        <div class="image-text_1 flex-row justify-between">
          <img
            class="label_6"
            referrerpolicy="no-referrer"
            src="./assets/img/FigmaDDSSlicePNGa704d75a097aefae9be9a77eff15ba5d.png"
          />
          <span class="text-group_1">作者的名字</span>
        </div>
        <div class="image-text_2 flex-row justify-between">
          <img
            class="thumbnail_1"
            referrerpolicy="no-referrer"
            src="./assets/img/FigmaDDSSlicePNG3628789c84a114c99145804a17f3aafb.png"
          />
          <span class="text-group_2">6512</span>
        </div>
      </div>
      <div class="group_8 flex-col"></div>
      <span class="text_10">APP广告变现痛点，开发者如何解决APP广告变现痛点，开发者如何解决？</span>
      <span class="text_11">
        如何在技术上实现广告植入的正常姿势，从而达成用户、流量、收益三者之间的动态平衡？如何在技术上实现广告植入的正常姿势，从而达成用户、流量、收益三者？
      </span>
      <div class="group_9 flex-row justify-between">
        <div class="image-text_3 flex-row justify-between">
          <img
            class="label_7"
            referrerpolicy="no-referrer"
            src="./assets/img/FigmaDDSSlicePNG148a64802181f491d49b1072f2b9dc22.png"
          />
          <span class="text-group_3">作者的名字</span>
        </div>
        <div class="image-text_4 flex-row justify-between">
          <img
            class="thumbnail_2"
            referrerpolicy="no-referrer"
            src="./assets/img/FigmaDDSSlicePNG3628789c84a114c99145804a17f3aafb.png"
          />
          <span class="text-group_4">6512</span>
        </div>
      </div>
      <div class="group_10 flex-col"></div>
      <span class="text_12">APP广告变现痛点，开发者如何解决APP广告变现痛点，开发者如何解决？</span>
      <span class="text_13">
        如何在技术上实现广告植入的正常姿势，从而达成用户、流量、收益三者之间的动态平衡？如何在技术上实现广告植入的正常姿势，从而达成用户、流量、收益三者？
      </span>
      <div class="group_11 flex-row justify-between">
        <div class="image-text_5 flex-row justify-between">
          <img
            class="label_8"
            referrerpolicy="no-referrer"
            src="./assets/img/FigmaDDSSlicePNGceceed640116808aaab41cc7616ad16e.png"
          />
          <span class="text-group_5">作者的名字</span>
        </div>
        <div class="image-text_6 flex-row justify-between">
          <img
            class="thumbnail_3"
            referrerpolicy="no-referrer"
            src="./assets/img/FigmaDDSSlicePNG3628789c84a114c99145804a17f3aafb.png"
          />
          <span class="text-group_6">6512</span>
        </div>
      </div>
      <div class="group_12 flex-col"></div>
      <span class="text_14">APP广告变现痛点，开发者如何解决APP广告变现痛点，开发者如何解决？</span>
      <span class="text_15">
        如何在技术上实现广告植入的正常姿势，从而达成用户、流量、收益三者之间的动态平衡？如何在技术上实现广告植入的正常姿势，从而达成用户、流量、收益三者？
      </span>
      <div class="group_13 flex-row justify-between">
        <div class="image-text_7 flex-row justify-between">
          <img
            class="label_9"
            referrerpolicy="no-referrer"
            src="./assets/img/FigmaDDSSlicePNGc95b064e7fe13debad45aa97c2610a0f.png"
          />
          <span class="text-group_7">作者的名字</span>
        </div>
        <div class="image-text_8 flex-row justify-between">
          <img
            class="thumbnail_4"
            referrerpolicy="no-referrer"
            src="./assets/img/FigmaDDSSlicePNG3628789c84a114c99145804a17f3aafb.png"
          />
          <span class="text-group_8">6512</span>
        </div>
      </div>
      <div class="group_14 flex-col"></div>
      <span class="text_16">APP广告变现痛点，开发者如何解决APP广告变现痛点，开发者如何解决？</span>
      <span class="text_17">
        如何在技术上实现广告植入的正常姿势，从而达成用户、流量、收益三者之间的动态平衡？如何在技术上实现广告植入的正常姿势，从而达成用户、流量、收益三者？
      </span>
      <div class="group_15 flex-row justify-between">
        <div class="image-text_9 flex-row justify-between">
          <img
            class="label_10"
            referrerpolicy="no-referrer"
            src="./assets/img/FigmaDDSSlicePNGc168a4131af9cbe754416bb43fa99c82.png"
          />
          <span class="text-group_9">作者的名字</span>
        </div>
        <div class="image-text_10 flex-row justify-between">
          <img
            class="thumbnail_5"
            referrerpolicy="no-referrer"
            src="./assets/img/FigmaDDSSlicePNG3628789c84a114c99145804a17f3aafb.png"
          />
          <span class="text-group_10">6512</span>
        </div>
      </div>
    </div>
    <div class="group_16 flex-col">
      <div class="list_1 flex-row">
        <div class="image-text_11 flex-col justify-between" v-for="(item, index) in loopData0" :key="index">
          <img class="image_4" referrerpolicy="no-referrer" :src="item.lanhuimage0" />
          <span class="text-group_11" v-html="item.lanhutext0"></span>
        </div>
      </div>
      <div class="group_17 flex-col"></div>
      <span class="paragraph_1">
        京ICP备19048506号-2&nbsp;Copyright
        <br />
        &#64;&nbsp;2024&nbsp;中国海洋财经有限公司&nbsp;All&nbsp;Rights&nbsp;Reserved.
      </span>
    </div>
    <div class="group_18 flex-col">
      <div class="text-wrapper_4 flex-row justify-between">
        <span class="text_18">今日资讯</span>
        <span class="text_19">Today's&nbsp;news</span>
      </div>
      <div class="box_3 flex-col"><div class="group_19 flex-col"></div></div>
      <div class="box_4 flex-col">
        <div class="box_5 flex-row justify-between">
          <div class="image-wrapper_5 flex-col">
            <img
              class="label_11"
              referrerpolicy="no-referrer"
              src="./assets/img/FigmaDDSSlicePNG5322eb3042173b371910d1f376d43dae.png"
            />
          </div>
          <img
            class="label_12"
            referrerpolicy="no-referrer"
            src="./assets/img/FigmaDDSSlicePNG51be87f91eb80dbabc2f36105767256c.png"
          />
        </div>
        <span class="text_20">中国新能源车20年：骗子、生死和“遥遥领先”</span>
        <div class="box_6 flex-row justify-between">
          <div class="image-text_12 flex-row justify-between">
            <img
              class="label_13"
              referrerpolicy="no-referrer"
              src="./assets/img/FigmaDDSSlicePNGf08e8cd84aef1eff64e468a4f34b1c41.png"
            />
            <span class="text-group_12">作者的名字</span>
          </div>
          <span class="text_21">2024-11-21&nbsp;11:09:15</span>
        </div>
      </div>
      <div class="box_7 flex-row justify-between">
        <div class="image-text_13 flex-col justify-between">
          <div class="group_20 flex-col">
            <div class="block_2 flex-row justify-between">
              <span class="text_22">作者：张三丰</span>
              <div class="image-text_14 flex-row justify-between">
                <img
                  class="thumbnail_6"
                  referrerpolicy="no-referrer"
                  src="./assets/img/FigmaDDSSlicePNG2565f79b9fead05d415e9e5ee0dcadf3.png"
                />
                <span class="text-group_13">6512</span>
              </div>
              <div class="image-text_15 flex-row justify-between">
                <img
                  class="thumbnail_6"
                  referrerpolicy="no-referrer"
                  src="./assets/img/FigmaDDSSlicePNG2565f79b9fead05d415e9e5ee0dcadf3.png"
                />
                <span class="text-group_13">6512</span>
              </div>
            </div>
          </div>
          <span class="text-group_14">APP广告变现痛点者如何解决？</span>
        </div>
        <div class="image-text_16 flex-col justify-between">
          <div class="group_21 flex-col">
            <div class="group_22 flex-row justify-between">
              <span class="text_23">作者：张三丰</span>
              <div class="image-text_17 flex-row justify-between">
                <img
                  class="thumbnail_7"
                  referrerpolicy="no-referrer"
                  src="./assets/img/FigmaDDSSlicePNG2565f79b9fead05d415e9e5ee0dcadf3.png"
                />
                <span class="text-group_15">6512</span>
              </div>
              <div class="image-text_18 flex-row justify-between">
                <img
                  class="thumbnail_7"
                  referrerpolicy="no-referrer"
                  src="./assets/img/FigmaDDSSlicePNG2565f79b9fead05d415e9e5ee0dcadf3.png"
                />
                <span class="text-group_15">6512</span>
              </div>
            </div>
          </div>
          <span class="text-group_16">APP广告变现痛点者如何解决？</span>
        </div>
      </div>
      <div class="text-wrapper_5 flex-row justify-between">
        <span class="text_24">2024-11-21&nbsp;11:09:15</span>
        <span class="text_25">2024-11-21&nbsp;11:09:15</span>
      </div>
      <div class="box_8 flex-row justify-between">
        <div class="image-text_19 flex-col justify-between">
          <div class="group_23 flex-col">
            <div class="box_9 flex-row justify-between">
              <span class="text_26">作者：张三丰</span>
              <div class="image-text_20 flex-row justify-between">
                <img
                  class="thumbnail_8"
                  referrerpolicy="no-referrer"
                  src="./assets/img/FigmaDDSSlicePNG2565f79b9fead05d415e9e5ee0dcadf3.png"
                />
                <span class="text-group_17">6512</span>
              </div>
              <div class="image-text_21 flex-row justify-between">
                <img
                  class="thumbnail_8"
                  referrerpolicy="no-referrer"
                  src="./assets/img/FigmaDDSSlicePNG2565f79b9fead05d415e9e5ee0dcadf3.png"
                />
                <span class="text-group_17">6512</span>
              </div>
            </div>
          </div>
          <span class="text-group_18">APP广告变现痛点者如何解决？</span>
        </div>
        <div class="image-text_22 flex-col justify-between">
          <div class="group_24 flex-col">
            <div class="box_10 flex-row justify-between">
              <span class="text_27">作者：张三丰</span>
              <div class="image-text_23 flex-row justify-between">
                <img
                  class="thumbnail_9"
                  referrerpolicy="no-referrer"
                  src="./assets/img/FigmaDDSSlicePNG2565f79b9fead05d415e9e5ee0dcadf3.png"
                />
                <span class="text-group_19">6512</span>
              </div>
              <div class="image-text_24 flex-row justify-between">
                <img
                  class="thumbnail_9"
                  referrerpolicy="no-referrer"
                  src="./assets/img/FigmaDDSSlicePNG2565f79b9fead05d415e9e5ee0dcadf3.png"
                />
                <span class="text-group_19">6512</span>
              </div>
            </div>
          </div>
          <span class="text-group_20">APP广告变现痛点者如何解决？</span>
        </div>
      </div>
      <div class="text-wrapper_6 flex-row justify-between">
        <span class="text_28">2024-11-21&nbsp;11:09:15</span>
        <span class="text_29">2024-11-21&nbsp;11:09:15</span>
      </div>
      <div class="box_11 flex-row justify-between">
        <div class="image-text_25 flex-col justify-between">
          <div class="box_12 flex-col">
            <div class="block_3 flex-row justify-between">
              <span class="text_30">作者：张三丰</span>
              <div class="image-text_26 flex-row justify-between">
                <img
                  class="thumbnail_10"
                  referrerpolicy="no-referrer"
                  src="./assets/img/FigmaDDSSlicePNG2565f79b9fead05d415e9e5ee0dcadf3.png"
                />
                <span class="text-group_21">6512</span>
              </div>
              <div class="image-text_27 flex-row justify-between">
                <img
                  class="thumbnail_10"
                  referrerpolicy="no-referrer"
                  src="./assets/img/FigmaDDSSlicePNG2565f79b9fead05d415e9e5ee0dcadf3.png"
                />
                <span class="text-group_21">6512</span>
              </div>
            </div>
          </div>
          <span class="text-group_22">APP广告变现痛点者如何解决？</span>
        </div>
        <div class="image-text_28 flex-col justify-between">
          <div class="box_13 flex-col">
            <div class="group_25 flex-row justify-between">
              <span class="text_31">作者：张三丰</span>
              <div class="image-text_29 flex-row justify-between">
                <img
                  class="thumbnail_11"
                  referrerpolicy="no-referrer"
                  src="./assets/img/FigmaDDSSlicePNG2565f79b9fead05d415e9e5ee0dcadf3.png"
                />
                <span class="text-group_23">6512</span>
              </div>
              <div class="image-text_30 flex-row justify-between">
                <img
                  class="thumbnail_11"
                  referrerpolicy="no-referrer"
                  src="./assets/img/FigmaDDSSlicePNG2565f79b9fead05d415e9e5ee0dcadf3.png"
                />
                <span class="text-group_23">6512</span>
              </div>
            </div>
          </div>
          <span class="text-group_24">APP广告变现痛点者如何解决？</span>
        </div>
      </div>
      <div class="text-wrapper_7 flex-row justify-between">
        <span class="text_32">2024-11-21&nbsp;11:09:15</span>
        <span class="text_33">2024-11-21&nbsp;11:09:15</span>
      </div>
      <div class="box_14 flex-row justify-between">
        <div class="image-text_31 flex-col justify-between">
          <div class="box_15 flex-col">
            <div class="section_3 flex-row justify-between">
              <span class="text_34">作者：张三丰</span>
              <div class="image-text_32 flex-row justify-between">
                <img
                  class="thumbnail_12"
                  referrerpolicy="no-referrer"
                  src="./assets/img/FigmaDDSSlicePNG2bc31857eddce9623a8f5ab51d489d1d.png"
                />
                <span class="text-group_25">6512</span>
              </div>
              <div class="image-text_33 flex-row justify-between">
                <img
                  class="thumbnail_12"
                  referrerpolicy="no-referrer"
                  src="./assets/img/FigmaDDSSlicePNG2bc31857eddce9623a8f5ab51d489d1d.png"
                />
                <span class="text-group_25">6512</span>
              </div>
            </div>
          </div>
          <span class="text-group_26">APP广告变现痛点者如何解决？</span>
        </div>
        <div class="image-text_34 flex-col justify-between">
          <div class="section_4 flex-col">
            <div class="section_5 flex-row justify-between">
              <span class="text_35">作者：张三丰</span>
              <div class="image-text_35 flex-row justify-between">
                <img
                  class="thumbnail_13"
                  referrerpolicy="no-referrer"
                  src="./assets/img/FigmaDDSSlicePNG2bc31857eddce9623a8f5ab51d489d1d.png"
                />
                <span class="text-group_27">6512</span>
              </div>
              <div class="image-text_36 flex-row justify-between">
                <img
                  class="thumbnail_13"
                  referrerpolicy="no-referrer"
                  src="./assets/img/FigmaDDSSlicePNG2bc31857eddce9623a8f5ab51d489d1d.png"
                />
                <span class="text-group_27">6512</span>
              </div>
            </div>
          </div>
          <span class="text-group_28">APP广告变现痛点者如何解决？</span>
        </div>
      </div>
      <div class="text-wrapper_8 flex-row justify-between">
        <span class="text_36">2024-11-21&nbsp;11:09:15</span>
        <span class="text_37">2024-11-21&nbsp;11:09:15</span>
      </div>
      <div class="box_16 flex-col"></div>
    </div>
  </div>
</template>
<script>
export default {
  data() {
    return {
      loopData0: [
        {
          lanhuimage0: 'https://lanhu.oss-cn-beijing.aliyuncs.com/FigmaDDSSlicePNGfcd97fd90f0f75097d6661238ccc7d77.png',
          lanhutext0: '咨询'
        },
        {
          lanhuimage0: 'https://lanhu.oss-cn-beijing.aliyuncs.com/FigmaDDSSlicePNGfcd97fd90f0f75097d6661238ccc7d77.png',
          lanhutext0: '关注'
        }
      ],
      constants: {}
    };
  },
  methods: {}
};
</script>
<style scoped lang="css" src="./assets/index.rem.css" />